...
class nbrAdminGrid{
  ...
  private function getValue($field, $record){
    ...
    switch ($field['type']) {
      ...
      case 'NUM':
        $value = number_format(utf8_encode($record->$field['fieldName']), 2, ',', '.');
        break;
  ...
  public function PrintHTML(){
    ...
    if($dataSet->ExistParam('msgError')){
      ...
      //Verifica de Mensagem de Erro tem detalhamento...
    ...
    if(count($this->records) > 0){
      ...
      foreach ($this->records as $x=>$record) {
        ...
        if($this->securityEdit){
          ...
          $hub->SetParam('_title', utf8_encode($record->$fieldTitle));
          $hub->SetParam('_description', 'Editando registro ' . utf8_encode($record->$fieldTitle) . ' da pasta ' . $this->title);
          $hub->SetParam('ID', $record->ID);
